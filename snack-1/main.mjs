...
bikes.forEach((bike) => {
  if (bike.bikeWeight < minWeightBike.bikeWeight) {
    minWeightBike = bike;
  }
});

const minWeightBikeHTML = `
  <div class="bike">
    <div class="image">
      <img src="${minWeightBike.bikeImage}" class="img-fluid" alt="" />
      <h2 class="text-center">${minWeightBike.bikeName}</h2>
      <h3 class="text-center">Peso: ${minWeightBike.bikeWeight}</h3>
    </div>
  </div>
`;

bikeContainer.innerHTML = minWeightBikeHTML;
